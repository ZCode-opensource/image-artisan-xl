# Copied and refactored from the diffusers library
# https://github.com/huggingface/diffusers/blob/main/src/diffusers/loaders.py
...
def convert_open_clip_checkpoint(
    ...
    **config_kwargs,
    ...
    with ctx():
        text_model = (
            CLIPTextModelWithProjection(config)
            if has_projection
...
def convert_ldm_clip_checkpoint(checkpoint, local_files_only=False, text_encoder=None):
    if text_encoder is None:
        config_name = "./configs/clip-vit-large-patch14"
        config = CLIPTextConfig.from_pretrained(
