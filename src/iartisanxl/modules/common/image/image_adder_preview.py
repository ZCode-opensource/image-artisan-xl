...
class ImageAdderPreview(QGraphicsView):
    ...
    #     if self.image_was_translated or self.image_was_scaled or self.image_was_rotated:
    #         dx = self.pixmap_item.x()
    #         dy = self.pixmap_item.y()
    #         angle = self.pixmap_item.rotation()

    #         self.image_convert_thread = ImageConvertThread(self.pixmap_item.pixmap())
    #         self.image_convert_thread.pixmap_converted.connect(self.on_pixmap_converted)
    #         self.image_convert_thread.start()
    ...
    #         if self.image_was_rotated:
    #             center = (width / 2, height / 2)
    #             pil_image = pil_image.rotate(-angle, Image.Resampling.BICUBIC, center=center, expand=True)

    #         if self.image_was_scaled:
    #             new_width = round(self.pixmap_item.sceneBoundingRect().width())
    #             new_height = round(self.pixmap_item.sceneBoundingRect().height())
    #             pil_image = pil_image.resize((new_width, new_height), Image.LANCZOS)

    #         if self.image_was_translated:
    #             left = math.floor(new_width / 2 - (width / 2 + dx))
    #             top = math.floor(new_height / 2 - (height / 2 + dy))
    #             right = round(self.mapToScene(self.viewport().rect()).boundingRect().width()) + left
    #             bottom = round(self.mapToScene(self.viewport().rect()).boundingRect().height()) + top
    #             pil_image = pil_image.crop((left, top, right, bottom))

    #         self.image_was_rotated = False
    #         self.image_was_scaled = False
    #         self.image_was_translated = False
    #         self.cropped_image = pil_image

    #     return self.cropped_image

    # def on_pixmap_converted(self, image: Image):
    #     dx = self.pixmap_item.x()
    #     dy = self.pixmap_item.y()
    #     angle = self.pixmap_item.rotation()
